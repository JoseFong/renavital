...
function CreateAnesthesia({ open, setOpen, reload }: { open: any, setOpen: any, reload: () => void }) {
    ...
    async function fetchCreate() {
        try {
            ...
            await axios.post("/api/anesthesias",data)
            toast.success("Tipo de anestesia creado exitosamente.")
            reload()
            setLoading(false)
            reset()
            setOpen(false)
    ...
    return (
        <Modal open={open} setOpen={setOpen}>
            <div className="flex flex-col gap-1">
                <h1 className="font-bold">Crear tipo de anestesia</h1>
                <label>Código</label>
...
export default CreateAnesthesia